...
static const uint8_t Crc8AtmLeft_table_[256] = {
...
static const uint16_t Crc16CcittRight_table_[256] = {
...
uint8_t Crc8AtmLeft(uint8_t crc, const unsigned char *data, const size_t size, const bool rev_flag) {
  ...
  for (i = 0; i < size; i++) {
    crc = Crc8AtmLeft_table_[crc ^ data[i]];
  }
...
uint16_t Crc16CcittRight(uint16_t crc, const unsigned char *c, const size_t n, const bool rev_flag) {
  size_t i;

  for (i = 0; i < n; ++i) {
    crc = (uint16_t)(Crc16CcittRight_table_[(crc ^ c[i]) & 0xff] ^ (crc >> 8));
  }
