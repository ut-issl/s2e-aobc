...
INA260::INA260(int prescaler, ClockGenerator *clock_gen,
               PowerPort *ina_power_port, const double ina_minimum_voltage,
               const double ina_assumed_power_consumption,
               PowerPort *observation_power_port, const int i2c_port_id,
...
INA260::INA260(INA260 &&obj) noexcept
    : Component(obj), I2cTargetCommunicationWithObc(std::move(obj)),
...
void INA260::GenerateTelemetry() {
  // Read PowerPort
  double current_mA =
      observation_power_port_->GetCurrentConsumption_A() * 1000.0;
  double voltage_mV = observation_power_port_->GetVoltage_V() * 1000.0;

  // Convert data
  signed short current_s16_mA =
      (signed short)(current_mA / kConvertObservedValue);
  ...
  for (int i = 0; i < kTlmSize; i++) {
    current_tlm_mA[i] =
        (unsigned char)(current_s16_mA >>
                        8 * (1 - i)); // current_tlm_tmp_mA[kTlmSize - i - 1];
    voltage_tlm_mV[i] =
        (unsigned char)(voltage_u16_mV >>
                        8 * (1 - i)); // voltage_tlm_tmp_mV[kTlmSize - i - 1];
  }

  WriteRegister((unsigned char)INA260_REGISTER::CURRENT, current_tlm_mA,
                kTlmSize);
...
void INA260::ReadConfigCmd() {
  ...
  if (rx_data[0] != (unsigned char)INA260_CMD::CONFIG)
    return;

  if (rx_data[1] == 0x62 && rx_data[2] == 0x07)
    mode_ = 0;
  return;
}
...
void INA260::ReadLimitMaskCmd() {
  unsigned char rx_data[3] = {0, 0, 0};
  ReadCommand(rx_data, 3);
  if (rx_data[0] != (unsigned char)INA260_CMD::LIMIT_MASK)
    return;

  if (rx_data[1] == 0x80 && rx_data[2] == 0x01) {
    observation_power_port_->SetCurrentLimit_A(over_current_threshold_mA);
  } else {
    observation_power_port_->SetCurrentLimit_A(
        10.0); // 絶対に引っかからない大きな値として10Aを入れる
  }
  return;
}
...
void INA260::ReadLimitValueCmd() {
  unsigned char rx_data[3] = {0, 0, 0};
  ReadCommand(rx_data, 3);
  if (rx_data[0] != (unsigned char)INA260_CMD::LIMIT_VALUE)
    return;
  ...
  over_current_threshold_mA =
      (double)(over_current_threshold_raw / kConvertLimitValue);
