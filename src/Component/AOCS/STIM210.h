...
public:
  STIM210(GyroSensor gyro, double compo_step_sec, const int sils_port_id,
          OnBoardComputer *obc);
  STIM210(GyroSensor gyro, double compo_step_sec, const int sils_port_id,
...
private:
   ...
   */
  typedef enum {
    OPERATION_INIT_MODE = 0,
    OPERATION_NORMAL_MODE,
    OPERATION_SERVICE_MODE,
    OPERATION_MODE_MAX
  } OPERATION_MODE;
  ...
  const uint8_t normal_mode_format_idx_[STIM210::NORMAL_MODE_MAX] = {
      0x90, 0x92, 0xa0, 0xa2, 0xa4,
      0xa5, 0x99, 0xa6, 0xa8}; //!< ノーマルモードでのテレメの1byte目に含まれるidx
  const uint32_t sample_rate_hz_[STIM210::SAMPLE_RATE_MAX] = {1,   125,  250,
                                                              500, 1000, 2000};
