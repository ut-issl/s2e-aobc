...
public:
  OEM7600(GnssReceiver gnssr, const int sils_port_id, OnBoardComputer *obc,
          const unsigned char oem_tlm_ch);
  OEM7600(GnssReceiver gnssr, const int sils_port_id, OnBoardComputer *obc,
          const unsigned char oem_tlm_ch, const unsigned int hils_port_id,
...
private:
  ...
  double last_position_fix_time_local_ =
      0; // to be removed after core_oss modification
  ...
  std::string
  Gen_TLM_Header_Ascii(const std::string tlm_name); // generate tlm header
  std::string
  Gen_BestXYZTlm_Ascii(void);          // function correspond to "bestxyza" tlm
  std::string Gen_TimeTlm_Ascii(void); // function correspond to "timea" tlm
  std::string
  Gen_HWMonitorTlm_Ascii(void);   // function correspond to "hwmonitora" tlm
  std::string Gen_GPGGATlm(void); // function correspond to "gpgga" tlm

  // TLM Packet Generator in Binary format
  std::string
  Gen_TLM_Header_Binary(const std::string tlm_name); // generate tlm header
  std::string
  Gen_BestXYZTlm_Binary(void); // function correspond to "bestxyzb" tlm
  std::string
  Gen_HWMonitorTlm_Binary(void); // function correspond to "hwmonitorb" tlm

  // CMD
  OEM7600_CMD DecodeCommand(); // command decoder
  int Cmd_LOG(const std::string comport_ID, const std::string tlm_name,
              const std::string type,
              const std::string freq); // decoder correspond to "log" command

  // CRC Calculation
  unsigned int OEM7600_calculate_crc32(const char *tlm_data_for_crc,
                                       const size_t data_length);
  unsigned int
  OEM7600_calculate_crc32_subroutine(const unsigned int initial_value);

  // Other utility functions for tlm/cmd string manipuration
  bool TLM_NameSearch(
      const std::string tlm_name); // check wether the designated tlm name in
                                   // "log" cmd is valid or not
  OEM7600_BINARY_TLM_ID GetTlmIdOfBinaryTlm(
      const std::string tlm_name); // get tlm id for binary format tlm packet
  unsigned short GetTlmLengthOfBinaryTlm(
      const std::string
          tlm_name); // get tlm length of variable part for binary format tlm

  std::string convLatLontoNMEA(
      const double rad,
      const std::string type); // convert lat/lon in [rad] to NMEA format
  std::string string_zeropad_local(
      const std::string str_org,
      const unsigned int len); // zero padding(to be removed and improved after
                               // core_oss modification)
  std::vector<unsigned char> ConvertDoubleToByte(
      const double double_data); // convert position,velocity data in double
                                 // format into byte format
  std::vector<unsigned char> ConvertFloatToByte(
      const float float_data); // convert position,velocity data in float format
                               // into byte format

  // TLM name list (additinoal tlm names will be added in the future as needed)
  const std::string tlm_name_dictionary_[OEM7600_MAX_TLM_LIST] = {
      "bestxyza", "timea", "gpgga", "hwmonitora", "bestxyzb", "hwmonitorb"};
};
