...
class OEM7600 : public GNSSReceiver, public ObcCommunicationBase
{
public:
  OEM7600(
    GNSSReceiver gnssr,
...
private:
  ...
  int Cmd_LOG(const std::string comport_ID, const std::string tlm_name, const std::string type, const std::string freq); // decoder correspond to "log" command
  ...
  std::string convLatLontoNMEA(const double rad, const std::string type);         // convert lat/lon in [rad] to NMEA format
  std::string string_zeropad_local(const std::string str_org, const unsigned int len);     // zero padding(to be removed and improved after core_oss modification)
  vector<unsigned char> ConvertDoubleToByte(const double double_data);  // convert position,velocity data in double format into byte format
  vector<unsigned char> ConvertFloatToByte(const float float_data);     // convert position,velocity data in float format into byte format
