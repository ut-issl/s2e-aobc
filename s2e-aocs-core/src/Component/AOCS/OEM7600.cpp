...
int OEM7600::GenerateTelemetry()
{
  ...
  for (size_t i = 0; i < tlm_list_.size(); i++)
  {
    ...
    if ((tlm_list_[i].count_to_out == 0)||((tlm_list_[i].out_type == "once")||(tlm_list_[i].out_type == "onnext")))
    {
      ...
      // delete "once" or "onnext" type tlm from the list so as not to be padded again without activation cmd
...
std::string OEM7600::Gen_TLM_Packet(const std::string tlm_name)
{
  ...
  if (tlm_name[tlm_name.length() - 1] == 'a')
  {
    ...
    if (tlm_name == "bestxyza") { str_tmp += Gen_BestXYZTlm_Ascii(); }
    else if (tlm_name == "timea") { str_tmp += Gen_TimeTlm_Ascii(); }
    else if (tlm_name == "hwmonitora") { str_tmp += Gen_HWMonitorTlm_Ascii(); }
    else
    {
      str_tmp = "";
  ...
  if (tlm_name[tlm_name.length() - 1] == 'b')
  {
    ...
    if (tlm_name == "bestxyzb") { str_tmp += Gen_BestXYZTlm_Binary(); }
    else if (tlm_name == "hwmonitorb") { str_tmp += Gen_HWMonitorTlm_Binary(); }
    else
    {
      str_tmp = "";
      return str_tmp;  // returns empty string
...
std::string OEM7600::Gen_HWMonitorTlm_Ascii(void)
{
  ...
  std::string str_tmp = "6,22.283273697,100,3.095238209,601,3.110134363,701,1.194749713,800,3.095238209,f01,1.821123362,1100*d59cf319";
  return str_tmp;
}
