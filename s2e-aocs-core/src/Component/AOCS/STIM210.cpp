...
STIM210::STIM210(
  ...
  OBC* obc
):Gyro(gyro), ObcCommunicationBase(sils_port_id, obc), counter_(0), compo_step_sec_(compo_step_sec)
{}
...
STIM210::STIM210(
  ...
  ObcCommunicationBase(sils_port_id, obc,hils_port_id, baud_rate, hils_port_manager), counter_(0),
...
void STIM210::GenerateFormatTlm(int& offset)
{
  ...
  vector<unsigned char> tlm = {normal_mode_format_idx_[normal_mode_format_]};
...
void STIM210::GenerateOmegaTlm(int& offset)
{
  ...
  const int kTlmSize = kOmegaByte * kGyroDim;
  vector<unsigned char> tlm(kTlmSize, 0); 
  int omega_c_tlm[kGyroDim] = {0,0,0};

  // TODO: LPFの実装

  for (size_t i=0; i<kGyroDim; i++)
  {
    omega_c_tlm[i] = ConvertOmega2Tlm(omega_c_[i]);
...
{
  const int kTlmSize = 1;
  vector<unsigned char> tlm = {status_};
  SetTlm(tlm, offset, kTlmSize);
  return;
}
...
void STIM210::GenerateTemperatureTlm(int& offset)
{
  ...
  const int kTlmSize = kTempByte * kGyroDim;
  vector<unsigned char> tlm(kTlmSize, 0);
  int temperature_c_tlm[kGyroDim] = {0,0,0};
  for (size_t i=0; i<kGyroDim; i++)
  {
...
{
  const int kTlmSize = 1;
  vector<unsigned char> tlm = {counter_};
  SetTlm(tlm, offset, kTlmSize);
  return;
}
...
{
  const int kTlmSize = 1;
  vector<unsigned char> tlm = {crc_8_atm_left(kCrcInitial, &tx_buffer_[0], offset, kRevFlag)};
  SetTlm(tlm, offset, kTlmSize);
  return;
}
...
{
  const int kTlmSize = 2;
  vector<unsigned char> tlm = {0x0d, termination_cr};
...
void STIM210::SetTlm(vector<unsigned char> tlm, int& offset, size_t tlm_size)
{
...
int STIM210::AnalyzeCmdServiceMode(vector<unsigned char> cmd)
{
...
int STIM210::AnalyzeCmdNormalMode(vector<unsigned char> cmd)
{
...
int STIM210::AnalyzeCmdTermination(vector<unsigned char> cmd)
{
  if (cmd[1] != ' ') return -1;
  switch (cmd[2])
  {
...
int STIM210::AnalyzeCmdSetNormalModeFormat(vector<unsigned char> cmd)
{
...
int STIM210::AnalyzeCmdSetSampleRate(vector<unsigned char> cmd)
{
  if (cmd[1] != ' ') return -1;
  int tmp = 1;
  // 文字数によって処理を変える
...
int STIM210::AnalyzeCmdSetOmegaMode(vector<unsigned char> cmd)
{
  if (cmd[1] != ' ') return -1;
  switch (cmd[2])
  {
...
int STIM210::AnalyzeCmdSetLPFFrequency(vector<unsigned char> cmd)
{
  if (cmd[1] != ' ') return -1;
  int tmp = 1;
  // 終端子の文字数によって処理を変える
